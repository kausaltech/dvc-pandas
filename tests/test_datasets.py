...
from dvc_pandas import has_dataset, load_dataset
...
def test_load_dataset_cached(upstream, local_cloud, cache, dataset):
...
def test_load_dataset_returns_cached_even_if_updated(upstream, dataset):
    df = load_dataset('dataset', upstream, cache_local_repository=True)
    df_updated = df.copy()
    df_updated['b'] += 1
